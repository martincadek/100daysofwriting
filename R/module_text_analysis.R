stopifnot("The object tidy_twdata does not exist." = exists("tidy_twdata"))
stopifnot("The object png_background does not exist." = exists("png_background"))
...
gg_lengths_and_dates <- ggplot(data = tidy_twdata, aes(x = created_at, y = display_text_width)) +
  ...
  annotation_custom(
    grob = rasterGrob(png_background),
    -Inf, Inf,
    60, 280
  ) +
...
# ggsave(
#   filename = "output/gg_lengths_and_dates.png", # will create folder if it does not exist
...
token_sentences <- tidy_twdata %>%
  ...
  mutate(
    sentences = clean_tokens(sentences, empty_as_na = TRUE),
...
date_ym_fct <- c(
  "2020 July", "2020 August", "2020 September",
...
ggplot(freq_token_words, aes(x = n)) +
  ...
  labs_100daysofwriting(
    x = "Frequency [word]", y = "Frequency [occurance]",
...
ggplot(freq_rank_token_words, aes(x = rank, y = word_frequency)) +
  ...
  geom_abline(
    intercept = -1.3370, slope = -0.6825,
    color = "#CC3333", linetype = 2
  ) +
  scale_x_log10() +
  scale_y_log10() +
  labs_100daysofwriting(
    x = "rank of word [log]", y = "frequnecy of word [log]",
...
ggplot(freq_token_words %>% slice_max(word, n = 15), aes(proportion, word)) +
  ...
  labs_100daysofwriting(
    title = "Relative frequency of words used during the challange",
    caption = "Top 15 words are selected. Note that word tokens 'day'/'days' were removed from the corpus."
  ) +
  theme_100daysofwriting()
...
ggplot(
  ...
  scale_y_continuous() +
  labs_100daysofwriting(
    title = "sample of sentences", y = "text width",
...
table_sentences <- sample_token_sentences %>%
  mutate(
    sentences = wrap_caption(sentences, max_width = 150),
...
sentiment_token_overall %>%
  ...
  labs_100daysofwriting(
    title = "sentiment of the most freqent words",
...
sentiment_average_month %>%
  ...
  labs_100daysofwriting(
    title = "Sentiment during the challange",
