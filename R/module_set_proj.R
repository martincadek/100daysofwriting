...
library(patchwork)
library(flextable)
library(textdata)       # NRC Word-Emotion Association Lexicon 
                        # Mohammad, Saif M. and Turney, Peter D.


# package versions --------------------------------------------------------
print("recording the session...")

sessionInfo() %>% capture.output(file = "docs/session_info.txt")
...
theme_100daysofwriting <- function(default_size = 40, title_family = "Proza", text_family = "Inconsolata") {
        ...
        dark_theme_minimal() +
                theme(
                        plot.title = element_text(family = title_family, hjust = .5, size = default_size,
...
labs_100daysofwriting <- function(x = NULL, y = NULL, title  = NULL, 
                                  ...
                                  caption_width = 100,
                                  tag  = NULL){
        labs(x = capitalisation_labeller(x), 
